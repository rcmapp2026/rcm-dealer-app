...
export const OffersView: React.FC<Props> = ({ offers, onRefresh }) => {
    ...
    return (
        <div className="bg-white min-h-screen pb-40 px-6 pt-8 font-bold">
            ...
            <AnimatePresence>
                {selectedOffer && (
                    <div className="fixed inset-0 z-[120] bg-white flex flex-col font-bold">
                        <header className="h-16 px-6 flex items-center justify-between border-b-2 border-slate-50">
                            <h2 className="text-sm font-bold text-black uppercase italic">Offer Details</h2>
                            <button onClick={() => setSelectedOffer(null)} className="h-10 w-10 bg-slate-50 rounded-xl flex items-center justify-center text-black">
                                <XCircle size={20} />
                            </button>
                        </header>
